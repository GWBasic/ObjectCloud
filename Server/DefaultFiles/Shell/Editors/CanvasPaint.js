...
// Some changes made by Andrew Rondeau in 2009 to be compatible with ObjectCloud
...
window.onload = function() { 	
    ...
    //alert(getPixel(0, 0));
		...
		// ADR:  The title bar never worked correctly in ObjectCloud...  It just covered up the image.
		// It really didn't have much functionality, so I just removed it
		...
		// Commented out because the file name is passed as a WebComponent
...
// ADR:  Edited to save to ObjectCloud
function save(image, mimetype)
{
   ...
   if (!dataurl)
   {
      alert("This browser can not save the image to a " + mimetype);
      return;
   }

   if (-1 == dataurl.indexOf(mimetype))
      alert("Your browser can not save as a " + mimetype + ", but instead saved as a " + dataurl.substring(0, dataurl.indexOf(",") + 1) + ".  The image file might need to be renamed in order to work correctly.");

   //hackish way to get to the Base64 encoded data
...
var paint = {
  ...
  open: function(url) {
    ...
    iface.status.innerHTML = 'Opening '+url+'...';
    img.onload = function() {
