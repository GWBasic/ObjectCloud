...
namespace ObjectCloud.Common.MIME
{
	...
	/// 
	/// </summary>
	public class MimeCodeQP : MIME.MimeCode
	{
		public MimeCodeQP()
		{
			// 
		...
		void DecodeOneLine(MemoryStream ms, string line)
		{
			...
			for(int i=0,j=0; i<line.Length; i++,j++)
			{
				...
				if(line[i] == '=')
				{
					...
					if(IsHex(line[i+1]) && IsHex(line[i+2]))
					{
		...
		public override string EncodeFromBytes(byte[] inArray, int offset, int length)
		{
			...
			return FormatEncodedString(enc.ToString());
		}
		...
		string FormatEncodedString(String s)
		{
			...
			try
			{
				while((line=sr.ReadLine()) != null)
				{
					...
					{
						if(IsHex(line[index]) && IsHex(line[index-1]) && line[index-2] == '=')
						{
							index -= 2;
						}
